...
class SphericalCamera(mi.Sensor):
    ...
    def __init__(self, props=mi.Properties()):
        ...
        self.m_near_clip = props.get('near_clip', 0.0)
    ...
    def sample_ray(self, time, wavelength_sample, position_sample, aperture_sample, active=True):
        ...
        wavelengths, wav_weight = None, 1.
        o = self.world_transform().translation()

        if self.use_sphere_uv:
            d = mi.warp.square_to_uniform_sphere(position_sample)
            d = self.world_transform() @ d
        else:
            sin_phi, cos_phi = dr.sincos(2 * dr.pi * position_sample.x)
            sin_theta, cos_theta = dr.sincos(dr.pi * position_sample.y)
            d = mi.Vector3f(sin_phi * sin_theta, cos_theta, -cos_phi * sin_theta)
            d = self.world_transform() @ d
        near_t = self.m_near_clip
        far_t = self.m_far_clip
        ray = mi.Ray3f(o, d, time)
        ray.o += ray.d * near_t
