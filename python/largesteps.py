...
def laplacian_2d(tex, lambda_=19, is_envmap=False):
    h,w = tex.shape[:2]
    N = h*w
    idx = np.arange(N)
    ...
    kk = np.stack((k,k))
    ll = np.stack((l,l))
    indices_1 = [ij,ji,kl,lk]
    indices_2 = [ii,jj,kk,ll]
    if is_envmap:
        m = idx[idx%w==0]
        n = idx[(idx+1)%w==0]
        mn = np.stack((m,n))
        ...
        mm = np.stack((m,m))
        nn = np.stack((n,n))
        indices_1.extend([mn,nm])
        indices_2.extend([mm,nn])
    indices = np.concatenate(indices_1 + indices_2, axis=1)
    values = np.ones(indices.shape[1])
    values[:-sum(x.shape[1] for x in indices_2)] = -1
...
class CholeskySolver():
    def __init__(self, x, lambda_, is_envmap=False):
        self.shape = x.shape
        self.channels = x.shape[-1]
        assert len(self.shape) in (3,4)
        self.N = dr.prod(self.shape[:-1])
        if len(self.shape) == 3:
            L_csc = laplacian_2d(x, lambda_, is_envmap=is_envmap)
        else:
            assert not is_envmap
